...
| ISS-003 | — | P1 | CI 失败：Clippy int_plus_one + items_after_test_module + Format 不合规 | push 触发 CI | Clippy: app.rs:864 int_plus_one / main.rs:133 items_after_test_module；Format: app.rs:132 单行 if 需拆多行 | CI 全绿 | 已修复 | dev-lead-ISS003-66 |
| ISS-004 | — | P2 | 移除快捷键提示栏后 `/` 搜索输入框无显示位置 | 在无底部快捷键提示栏的版本中按 `/` 进入搜索模式 | 搜索输入框没有区域可以展示，UI 无响应位置 | 搜索输入框应在界面某区域正常展示 | 已修复 | dev-lead-ISS004-1 |
