...
pub fn render_list(frame: &mut Frame, app: &AppState, area: Rect) {
    ...
    let idx = if total == 0 { 0 } else { app.selected_index + 1 };
    ...
    let items: Vec<ListItem> = filtered.iter().map(|todo| {
        ...
        let title_style = match todo.status {
            TodoStatus::Done | TodoStatus::Cancelled => {
                Style::default().fg(Color::Rgb(100, 100, 100)).add_modifier(Modifier::CROSSED_OUT)
            }
            TodoStatus::Pending if todo.is_overdue() => {
                Style::default().fg(Color::Rgb(220, 100, 40))
            }
        ...
        if let Some(d) = todo.due_date.as_deref() {
            ...
            let (text, style) = if todo.is_overdue() {
                (
                    format!("⚠{}", short),
                    Style::default().fg(Color::Rgb(220, 100, 40)).add_modifier(Modifier::BOLD),
                )
